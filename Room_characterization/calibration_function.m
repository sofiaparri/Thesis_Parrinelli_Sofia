...
addpath(genpath(pwd))
    ...
    folderPath_calibrated = 'IR_centrale_original_sweep'; 
 ...
 if(choice ~= 9 && choice ~= 11 )
    ...
    for i=1:n_ls
        ...
        audiowrite(fullfile(folderPath_calibrated, ['IR', int2str(i), '.wav']), IR,  fs, 'BitsPerSample',32);
   
        %audiowrite(fullfile(folderPath_calibrated, ['IR', int2str(i), '.wav']), IR,  fs);
 ...
 % PLOT
     ...
     semilogx(f,Z, 'black')
     title([ num2str(i)]); 
     xticks([  100 500 1000 2000 4000]) ;
     xticklabels({'100','500','1k','2k', '4k'});
     xlim( [ 30 24000 ] )
     ylim( [50 max(Z)+15] )
     ax = gca;
     ax.XGrid = 'on';
...
%
   ...
   xticks([ 100 200 500 1000 2000 4000 10000])
   xticklabels({'100','200','500','1000','2000', '4000', '10000'})
    audiowrite(fullfile(folderPath_calibrated, 'sum_calibrated_wholeCover.wav'), rescale(total_sum_calibrated_cut, -1, 1), fs);
   return;
 end 
