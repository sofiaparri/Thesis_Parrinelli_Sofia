...
% ita_soundInsulationIndexAirborne - impact sound ratings acc. to ISO 717-2 or ASTM E989/E3222
...
elseif strcmpi(sArgs.type,'astm') % Reference curve and frequencies according to ASTM E989
    ...
    roundingFactorHIIC = 0.1;
    refCurveHIIC = refCurve(7:end);
    refSurfHIIC = 20;
else
...
if strcmpi(sArgs.type,'astm')
    ...
    % high-frequency rating according to ASTM E3222
    NISPL_HIIC = round(20.*log10(interp1(freqVector,data.freqData(:,1),freq(7:end),'spline','extrap')) + 93.98,-log10(roundingFactorHIIC));
    counter = 0;
    impactInsulationClassH = max(ceil(NISPL_HIIC - refCurveHIIC));
    deltaHIIC = max(0,NISPL_HIIC - (refCurveHIIC + impactInsulationClassH));
