...
h.baseFreqText = uicontrol('style', 'text', 'parent', h.f, 'units', 'normalized', 'position', [17 14.5  1.8 0.3]/20,'string',['Sine Frequency: ' num2str(data.mainSineFreq) ' Hz']);
...
function guiData = populateGUI(guiData)
    ...
    if data.plotSine
        data.sinePlot = plot(h.ax_pos3d,sineTime,'LineWidth',4,'Color',[0,0.447,0.741]);
    end
    hold all
...
%     sineFreq = str2double(get(data.sineFreqEdit,'String'));
    ...
    if (data.plotSampling)
        % stem plot
        data.stemPlot = stem(x,y,'LineWidth',3,'Color',[0.4940    0.1840    0.5560]);
    ...
    if (data.plotReconstructed)
        ...
        if (abs(gof.rsquare - 1)) < 0.1 % good fit
            x2 = data.xAxis;
            data.interpolatedPlot = plot(h.ax_pos3d,x2,f.a1*sin(f.b1*x2+f.c1),...
...
function freqEditCallback(hObject,~)
    ...
    if newValue > guiData.handles.freqSlider.Max
        newValue = guiData.handles.freqSlider.Max;
    end
    
    if newValue < guiData.handles.freqSlider.Min
        newValue = guiData.handles.freqSlider.Min;
    end
    
    guiData.data.freqValue = newValue;
    set(guiData.handles.freqEdit,'string',num2str(guiData.data.freqValue));
