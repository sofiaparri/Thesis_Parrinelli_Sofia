...
%% Init
ccx
mode = 3; % 1: simple RIR, 2: modal superposition, 3: read your measurement data from harddrive
...
if mode == 2
    ...
    RIR         = RIR + 10^ (-(PSNR-10) / 20) *ita_generate('noise', 1, samplingRate, fftDegree);  % add background noise (10 dB difference between peak and mean)
    RIR.plot_dat_dB
    title('RIR from modal superposition in rectangular room')
end
...
freqRange       = [80 2500];       % frequency range and
bandsPerOctave  = 3;                % bands per octave for filtering

% calculate just T20 with given frequency range in 1/3 octave bands
...
raResult.T20.channelNames = {'Octave bands of Reverberation time T20'};
raResult.T20.bar
...
raResults = ita_roomacoustics(RIR, 'freqRange', freqRange, 'bandsPerOctave', bandsPerOctave, 'PSNR_Lundeby', 'EDT', 'T20', 'C80', 'PSNR_Lundeby', 'Center_Time' );
% output is a struct with itaResults:
raResults.EDT.channelNames = {'Octave bands'};
raResults.T20.channelNames = {'Octave bands'};
raResults.C80.channelNames = {'Octave bands'};
raResults.PSNR_Lundeby.channelNames = {'Octave bands'};
raResults.Center_Time.channelNames = {'Octave bands'};
...
title('Energy parameter C80')
raResults.PSNR_Lundeby.plot_freq
title('Lundeby method')
raResults.Center_Time.plot_freq
title('Center_Time')
...
allRes.comment = 'Differences of noise compensation techniques';
