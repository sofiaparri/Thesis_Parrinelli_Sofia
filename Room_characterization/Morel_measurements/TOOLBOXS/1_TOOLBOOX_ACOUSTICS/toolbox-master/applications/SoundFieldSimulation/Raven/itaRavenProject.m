classdef itaRavenProject < handle
    ...
    methods
        ...
        function loadRavenConfig(obj, filename)
            ...
            if (exist(obj.fileHRTF,'file') == 0) 
                ...
                if (contains(obj.fileHRTF,'.v17.daff') || contains(obj.fileHRTF,'.v17.ir.daff'))
                   disp('itaRavenProject: Loaded DAFFv17 HRTF file'); 
                end
                if (contains(obj.fileHRTF,'.v15.daff') || contains(obj.fileHRTF,'.v15.ir.daff'))
            ...
            if (~isempty(obj.sourceDirectivity)) % first check if any directivity was defined
                if (exist([ obj.pathDirectivities '\' obj.sourceDirectivity{1}],'file') == 0) 
                    ...
                    if (contains(obj.sourceDirectivity{1},'.v17.daff') || contains(obj.sourceDirectivity{1},'.v17.ms.daff'))
                       disp('itaRavenProject: Loaded DAFFv17 source directivity file'); 
                    end
                    if (contains(obj.sourceDirectivity{1},'.v15.daff') || contains(obj.sourceDirectivity{1},'.v15.ms.daff'))
        ...
        function plotMaterialsAbsorption(obj, exportPlot, fileType, outputPath)
            ...
            for iMat=1:numberMaterials
                ...
                allMaterials{iMat} = [ allMaterials{iMat} ' A (f=1000 Hz) = ' num2str(currentSurfaceArea*currentMaterial.freqData(18,iMat),'%5.2f') ' m² )'];
            end
        ...
        function setNumParticles(obj, num_particles)
            if num_particles~=round(num_particles)
                warning(['Number of particles rounded to: ' num2str(ceil(num_particles))]);
            end
            obj.numParticles_Sphere = ceil(num_particles);
            obj.rpf_ini.SetValues('RayTracing', 'numberOfParticles_DetectionSphere', ceil(num_particles));
            obj.rpf_ini.WriteFile(obj.ravenProjectFile);
        end
        ...
        function setNumParticles_Portal(obj, num_particles)
            ...
            obj.numParticles_Portal = ceil(num_particles);
            obj.rpf_ini.SetValues('RayTracing', 'numberOfParticles_Portal', ceil(num_particles));
            obj.rpf_ini.WriteFile(obj.ravenProjectFile);
        end
    ...
    methods (Access = 'private')
        %------------------------------------------------------------------
        function [numFilesIR,numFilesOther] = gatherResults(obj)
    ...
    methods(Static)
        ...
        function rvn_pos = pSU2RVN(su_pos)
            ...
            rvn_pos = su_pos;
            rvn_pos(:, 1) = sign(itaRavenProject.COORD_TRAFO_SKETCHUP2RAVEN(1)) .* su_pos(:, abs(itaRavenProject.COORD_TRAFO_SKETCHUP2RAVEN(1)));
            rvn_pos(:, 2) = sign(itaRavenProject.COORD_TRAFO_SKETCHUP2RAVEN(2)) .* su_pos(:, abs(itaRavenProject.COORD_TRAFO_SKETCHUP2RAVEN(2)));
            rvn_pos(:, 3) = sign(itaRavenProject.COORD_TRAFO_SKETCHUP2RAVEN(3)) .* su_pos(:, abs(itaRavenProject.COORD_TRAFO_SKETCHUP2RAVEN(3)));
        end
        ...
        function su_pos = pRVN2SU(rvn_pos)
            ...
            su_pos = rvn_pos;
            su_pos(:, 1) = sign(itaRavenProject.COORD_TRAFO_RAVEN2SKETCHUP(1)) .* rvn_pos(:, abs(itaRavenProject.COORD_TRAFO_RAVEN2SKETCHUP(1)));
            su_pos(:, 2) = sign(itaRavenProject.COORD_TRAFO_RAVEN2SKETCHUP(2)) .* rvn_pos(:, abs(itaRavenProject.COORD_TRAFO_RAVEN2SKETCHUP(2)));
            su_pos(:, 3) = sign(itaRavenProject.COORD_TRAFO_RAVEN2SKETCHUP(3)) .* rvn_pos(:, abs(itaRavenProject.COORD_TRAFO_RAVEN2SKETCHUP(3)));
        end
