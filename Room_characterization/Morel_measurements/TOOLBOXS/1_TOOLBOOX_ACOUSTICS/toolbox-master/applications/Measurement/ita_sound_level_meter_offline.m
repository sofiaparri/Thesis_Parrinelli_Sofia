...
[fc,fc_ex] = deal(ita_ANSI_center_frequencies(sArgs.freqRange,sArgs.bandsPerOctave,samplingRate).');
[~,Beff] = statistical_filter_bandwidth(fc_ex,1/sArgs.bandsPerOctave,filterOrder/2);
weighting = itaResult(Beff./(sum(abs(oct_filt.freq).^2).'/oct_filt.trackLength),fc,'freq')^2;
