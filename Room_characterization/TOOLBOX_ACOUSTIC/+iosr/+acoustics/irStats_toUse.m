function [rt,drr,cte, cte_mean, cfs,edt] = irStats_toUse(filename,varargin)
...
%   Copyright 2016 University of Surrey.
    ...
    cfs = [ 62.5 125 250 500 1000 2000 4000 8000];
    %cfs = [1000 ];
    ...
    cte = zeros([length(cfs) numchans]);      %takes frequency bands ir
    cte_mean = zeros(1, numchans); %takes the whole ir
    ...
    for n = 1:numchans
        ...
        if nargout>=3
            if t0(n)+te+1>size(x,1)
             ...
             for f = 1:length(cfs)
             y = filter(b(f,:),a(f,:),x(:,n));
                %in frequency bands
                cte(f, n) = 10.*log10(...
                    trapz(y(max(1,t0(n)-correction):t0(n)+te).^2)/...
                    trapz(y(t0(n)+te+1:end,n).^2)...
                    );
             end
             cte_mean(n) = 10.*log10(...
