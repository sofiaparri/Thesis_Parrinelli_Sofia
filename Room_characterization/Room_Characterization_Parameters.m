...
switch choice
    ...
    case 12
        opzione=3;
        disp('Option defined in the code');
        file_path='C:\Users\User\Desktop\TESI_POLIMI\Thesis_Parrinelli_Sofia\Ambisonics_assessment\2024.02.26 - Misure VR-PCBmic\MeasureTest\Sweep_22-22k_0.2_0.2_10s+2s_-28dB_fLow80Hz'; 
        calRec=audioread('C:\Users\User\Desktop\TESI_POLIMI\Thesis_Parrinelli_Sofia\Ambisonics_assessment\2024.02.26 - Misure VR-PCBmic\MeasureTest\Silence10s\CalibrationMic_1.wav');
...
elseif choice==12
    [inv_sweep, fs]=audioread('C:\Users\User\Desktop\TESI_POLIMI\Thesis_Parrinelli_Sofia\Ambisonics_assessment\2024.02.26 - Misure VR-PCBmic\invSweep_22-22k_0.2_0.2_10s+2s_-6dB.wav');
else
...
%% parameters extraction and visualization- parameters extraction trought irStat - give the right path
    [rt,drr, cte, cte_mean, cfs, edt]=iosr.acoustics.irStats_toUse('sum_calibrated_wholeNOCover.wav', 'graph', true, 'y_fit', [-5 -25]);
    [rt_b,drr_b, cte_b, cte_mean_b, cfs_b, edt_b]=iosr.acoustics.irStats_toUse('central_cover.wav', 'graph', true, 'y_fit', [-5 -25]);

    rt_tot=zeros([2 length(cfs)]);
    rt_tot(1, :)=rt;
    rt_tot(2, :)=rt_b;
    %for i=1:length(cfs)
    %   if i<=4
    ...
    X = categorical(cfs(1:end));
    figure();
    bar(X, rt_tot);
    %legend('Subwoofer excitation', 'Woofers and tweeters excitation ');
    %title({'Reverberation Time T20', ' Sweep signals. TV and door not covered'});
    legend('Sweep ', 'Balloon');
    title({'Reverberation Time T20', ' TV and door not covered'});
    ylabel('Time [s]');
    ...
    bar(X, edt_tot);
    %legend('Subwoofer excitation', 'Woofers and tweeters excitation ');
    %title({'EDT', ' Sweep signals. TV and door not covered'});
    legend('Sweep ', 'Balloon');
    title({'EDT', ' TV and door not covered'});
    ylabel('Time [s]');
    xlabel('Octave band [Hz]');
    ...
    ylabel('dB');
    legend('Sweep ', 'Balloon');
    title({'C50', ' TV and door not covered'});
    %title({'C50',' Sweep signals. TV and door not covered'});
    xlabel('Octave band [Hz]');
    %% parameters extraction and visualization- parameters extraction trought ITA Toolbox
    RIR = ita_read;

    freqRange       = [20 20000];       % frequency range and
