classdef itaHpTF_MS < itaHpTF
    %ITAHPTF_MS - Measurement of the HpTF in accordance with:
    % Masiero, Bruno; Fels, Janina: "Perceptually Robust Headphone 
    % Equalization for Binaural Reproduction" -- 
    % Audio Engineering Society Convention 130, May 2011)
    %
    % Usage:
    % hptfMS = itaHpTF_MS() - calls GUI
    % hptfMS = itaHpTF_MS(itaMSTF) - setup without GUI call
    %
    % to measure HpTF call:
    % HpTF = hptfMS.run
    % HpTF = hptfMS.run('Name of Participant')
    % compensationFilter = HpTF.HP_equalization;
    %
    ...
    %                       also be used with the subjects' name: 
    %                       this.run('Hans'). 
    %                       This method will return an itaHpTF_Audio to the
    %                       workspace. 
    ...
    methods
        function this = itaHpTF_MS(varargin)
            ...
            this = this@itaHpTF(); %call super constructor
