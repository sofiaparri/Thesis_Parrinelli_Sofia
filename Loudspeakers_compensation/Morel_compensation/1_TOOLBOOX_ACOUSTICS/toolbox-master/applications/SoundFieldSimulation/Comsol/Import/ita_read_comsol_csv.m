...
%   strrep ({}) : {'originalString','replacementString'}
%                 used for replacing strings i.e. from using formulas in
%                 export of comsol file
%
%   Output:
...
if ~isempty(sArgs.strrep)
    columnHeaders = strrep(columnHeaders,sArgs.strrep{1},sArgs.strrep{2});
end
...
%% Check header data
% moved up to spot parsing errors without needing to load data
...
for idxDataSet = 1:numel(resultData)
    ...
    idxInvalidFrequecies = all(isnan(freqDataOfCurrentVariable), 1);
