...
weightingFilters = [flat,abs(weighting_filter_local(flat,octFreqVec(2:end),'A')).^2,abs(weighting_filter_local(flat,octFreqVec(2:end),'C')).^2];
weightingLabels = {'','(A)','(C)'};
...
function input = weighting_filter_local(input,freqVector,type)
...
factor = f4^2 .* freqVector(:).^2./((freqVector(:).^2 + f1.^2).*(freqVector(:).^2 + f4.^2));
switch lower(type)
    case 'a'
        weightingFilter = factor.*freqVector(:).^2./(sqrt(freqVector(:).^2 + f2.^2).*sqrt(freqVector(:).^2 + f3.^2)) .* 10.^(-A1000./20);
    case 'c'
        weightingFilter = factor.* 10.^(-C1000./20);
    otherwise
        error(['Wrong weighting type:' type]);
end
input = bsxfun(@times,input,weightingFilter);
end
